...
## Project Layout (refactor branch)
- `apps/api` — основной FastAPI сервис (роутеры, шаблоны, статика, расширения Starlette/FastAPI).
- `apps/worker` — Telegram/WhatsApp воркер и вспомогательные entrypoints (`main.py`, `http.py`, `outbox.py`).
- `apps/tgworker` — отдельный сервис авторизации/отправки Telegram (FastAPI + Telethon).
- `apps/waweb`, `apps/wabaileys` — ноды для WhatsApp транспорта (puppeteer и Baileys).
- `libs/core` — общий код (core settings, БД, каталоги, интеграции, transport utils и т.д.).
...
- Тестовый диалог (`POST /api/dialogs/test`) использует этот пайплайн, чтобы ответ совпадал с боевым.
- В воркере включается флагом `RESPONSE_PIPELINE_ENABLED=1` (по умолчанию выключен, остаётся старый путь).
...
- Две ветки и стенда:
  ...
  - `dev` → деплой в `/opt/avio-dev` (дев). Здесь может лежать прототип SPA; его не мёржим в `prod`, пока не решено катить.
- Рабочий цикл:
...
- В секции `meta` можно указать дополнительные артефакты:
  - `meta.catalog_pdf_path` — относительный путь (от корня `tenants/<ID>/`) до PDF каталога. Этот файл автоматически подставляется при первом сообщении из WhatsApp/Telegram и при явном запросе клиента в любом канале.
...
- **Правила каталога и нужд.** За бизнес-ограничения теперь отвечает `persona.md → meta`:
...
### WhatsApp
- **Первое сообщение всегда включает ссылку на viewer**. Берётся из `data/tenants/<id>/tenant.json → integrations.catalog_url` (или `persona.meta.catalog_url`). Если ссылка не задана, используется fallback `https://api.avio.website/pub/catalog/file/<tenant>` с параметром `?v=<timestamp>`.
- PDF и постраничные каталоги в WA больше не отправляются: `_handle_catalog_flow()` принудительно переключается на текстовую ссылку, даже если в `behavior` включены `always_full_catalog` или `send_catalog_as_pages`.
...
- Если ссылка приходит локальная (`http://app:8000/...`), проверьте, что контейнер перезапущен после изменения tenant-конфига и что `integrations.catalog_url` заполнен внешним URL.

### Avito
- В `persona.md` для Avito должен быть CTA с переводом в WhatsApp. Типовой сценарий: каждое сообщение содержит номер `wa.me`.
- Avito-сообщения также проходят через `process_incoming`, но автоматическая отправка каталога отключена для этого канала.

### Telegram
- Логика каталога такая же, как в WA, но PDF остаётся разрешён (если файл небольшой). Флаг `send_catalog_as_pages` продолжает работать для Telegram.

### Настройка tenant.json
- `integrations.catalog_url` — основная viewer-ссылка.
- `behavior`:
  - `always_full_catalog`, `send_catalog_as_pages` — работают в Telegram/Avito, но игнорируются в WhatsApp.
