...
# Получение PNG с QR-кодом (ссылка приходит из /start)
...
- `waiting_qr` — QR сгенерирован и ждёт сканирования.
...
|---------|----------|-----------------|-------------|
| `GET /pub/tg/start` | Запускает получение QR. Возвращает JSON/HTML с `qr_url`. | `200` | `409 already_authorized`, `502` при `qr_expired`/`failed` |
| `GET /pub/tg/qr.png` | Отдаёт PNG текущего QR. | `200` | `404 qr_not_found`, `410 qr_expired`, `502 tg_unavailable` |
| `GET /pub/tg/status` | Снимок статуса (`authorized`, `state`, `needs_2fa`). | `200` | `502 tg_unavailable` |
...
1. Вызвать `GET /pub/tg/start?k=${PUBLIC_KEY}&tenant=<TENANT_ID>` и сохранить `qr_url` из ответа.
2. Отображать QR с помощью `GET /pub/tg/qr.png?k=${PUBLIC_KEY}&tenant=<TENANT_ID>`.
3. Параллельно опрашивать `GET /pub/tg/status` до получения `authorized=true`.
