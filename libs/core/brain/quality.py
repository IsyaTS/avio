...
def _dedupe_repeated_blocks(text: str) -> str:
    ...
    Questions that differ незначительно (CTA-хвост, уточняющие слова) считаются
    дублями, если их токеновые множества заметно перекрываются.
    """
    ...
    for raw_line in lines:
        ...
        if "?" in line:
            ...
            if tokens:
                duplicate = False
                for prev_tokens in seen_questions:
                    overlap = len(tokens & prev_tokens)
                    if not overlap:
                        continue
                    coverage_cur = overlap / max(1, len(tokens))
                    coverage_prev = overlap / max(1, len(prev_tokens))
                    jaccard = overlap / max(1, len(tokens | prev_tokens))
                    if coverage_cur >= 0.6 or coverage_prev >= 0.6 or jaccard >= 0.55:
                        duplicate = True
                        break
                if not duplicate:
                    for prev_tokens in seen_sentence_tokens:
                        overlap = len(tokens & prev_tokens)
                        if not overlap:
                            continue
                        coverage_cur = overlap / max(1, len(tokens))
        ...
        tokens = set(_tokenize(line))
        duplicate = False
        for prev_tokens in seen_sentence_tokens + seen_questions:
            overlap = len(tokens & prev_tokens)
            if not overlap:
                continue
            coverage_cur = overlap / max(1, len(tokens))
            coverage_prev = overlap / max(1, len(prev_tokens))
            jaccard = overlap / max(1, len(tokens | prev_tokens))
            if coverage_cur >= 0.65 or coverage_prev >= 0.65 or jaccard >= 0.55:
                duplicate = True
                break
        if duplicate:
            continue
