...
core_settings = getattr(core_module, "settings", None)
...
async def send(
    *,
    ...
    phone:
        Optional recipient phone (used by worker to resolve/import a contact).
    attachments:
    ...
    if not peer_value and not phone_value and lead_id is not None and lead_id > 0:
        try:
    ...
    if not peer_value and not phone_value:
