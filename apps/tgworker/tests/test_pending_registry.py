...
from apps.tgworker.api import create_app
from apps.tgworker.session_manager import (
...
def stub_manager(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("apps.tgworker.api.SessionManager", lambda *args, **kwargs: manager)
    monkeypatch.setattr("apps.tgworker.api.telegram_config", _fake_config)

    monkeypatch.setattr("apps.tgworker.api.time.time", lambda: manager.now)
