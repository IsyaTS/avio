...
from apps.tgworker.manager import NotAuthorizedError
...
def tgworker_app(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("apps.tgworker.api.ADMIN_TOKEN", "test-token")
    monkeypatch.setattr(
        "apps.tgworker.api.telegram_config",
    ...
    dummy = DummyManager()
    monkeypatch.setattr("apps.tgworker.api.SessionManager", lambda *args, **kwargs: dummy)
    app = create_app()
