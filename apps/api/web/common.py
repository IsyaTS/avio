...
def wabaileys_base_url() -> str:
    base = getattr(settings, "BAILEYS_URL", "") or (
        os.getenv("BAILEYS_URL") or os.getenv("WABAILEYS_URL") or "http://wabaileys:9002"
    )
    return str(base).rstrip("/")


def whatsapp_provider(tenant: int | None) -> str:
    return tenant_whatsapp_provider(tenant)
...
async def wabaileys_post(
    path: str,
    data: dict,
    *,
    timeout: float = 8.0,
) -> httpx.Response:
    url = f"{wabaileys_base_url()}{path}"
    headers = {"Content-Type": "application/json; charset=utf-8"}
    async with httpx.AsyncClient(timeout=timeout) as client:
        last_response: httpx.Response | None = None
        last_error: Exception | None = None
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError("wabaileys_post failed without response")


def wabaileys_http(method: str, path: str, body: bytes | None = None, timeout: float = 8.0):
    url = f"{wabaileys_base_url()}{path}"
    return http(method, url, body=body, timeout=timeout)
